...
import {
  ...
  CreditCard, Calendar, Star, TrendingUp,
  Shield, Heart, Sparkles, Gift, FileText,
  AlertTriangle, Scale, RefreshCw, Mail,
} from "lucide-react";
...
const REVIEWS = [
  ...
  { name:"Sana Sheikh",   city:"Islamabad", text:"Online shop ke liye paisa chahiye tha. Bilkul interest-free loan mila.", amount:"10,000", stars:5, tag:"Entrepreneur" },
];
...
async function submit(e){
  ...
  async function checkAdmin(){
    if(!adminEmail.trim()||!adminPass.trim()){
      setPassErr("Email aur password dono likhein.");
      return;
    }
    setPassErr("");
    ...
    try{
      ...
      setPassErr("Ghalat email ya password.");
    }
    ...
    <div style={{minHeight:"100vh",background:"#FAFDFB",color:"#111827",fontFamily:"'Inter',sans-serif"}}>
      ...
      {view==="landing"&&<>
        <section style={{background:"linear-gradient(180deg,#F0FDFA,#FAFDFB)",padding:"48px 24px 44px",textAlign:"center"}}>
          ...
          <div style={{marginTop:28,display:"inline-flex",alignItems:"center",gap:8,background:"#FFF",border:"1px solid #99F6E4",borderRadius:24,padding:"8px 18px",fontSize:13,color:"#0F766E",fontWeight:600}}>
            100% Halaal — Bilkul ZERO Interest
          </div>
        ...
        <div style={{maxWidth:800,margin:"0 auto",padding:"0 24px"}}>
          <div style={{background:"linear-gradient(135deg,#F0FDFA,#ECFDF5)",border:"1px solid #99F6E4",borderRadius:18,padding:"22px 26px",display:"flex",alignItems:"center",gap:18}}>
            <Shield size={32} color="#0D9488" style={{flexShrink:0}}/>
            <div>
        ...
        <div style={{maxWidth:900,margin:"44px auto 0",padding:"0 24px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:16}}>
            {[
              {title:"Students",     desc:"Fees, books, laptop — padhai ke liye halaal madad.",  color:"#0D9488",bg:"#F0FDFA"},
              {title:"Khawateen",    desc:"Ghar ke kharche ya apna business shuru karein.",      color:"#7C3AED",bg:"#FAF5FF"},
              {title:"Zarooratmand", desc:"Emergency medical ya unexpected bills ke liye.",      color:"#EA580C",bg:"#FFF7ED"},
            ].map((c,i)=>(
        ...
        <div style={{maxWidth:600,margin:"36px auto 0",padding:"0 24px"}}>
          <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:16,padding:"20px 24px"}}>
            ...
            <div style={{fontSize:13,color:"#78716C",lineHeight:1.75}}>
              CNIC Verification, Background Check, Documentation, SMS and Call Updates.<br/><br/>
              Loan mein koi interest <strong>NAHI</strong> hai.
            </div>
          </div>
        </div>
        ...
        <footer style={{borderTop:"1px solid #F3F4F6",padding:"28px 24px",textAlign:"center",marginTop:44}}>
          <button onClick={()=>setView("terms")} style={{background:"none",border:"none",color:"#0D9488",fontSize:13,cursor:"pointer",fontWeight:600,marginBottom:10,display:"flex",alignItems:"center",gap:4,margin:"0 auto 10px"}}>
            <FileText size={13}/> Terms and Conditions
          </button>
      ...
      {view==="terms"&&(
        <div style={{maxWidth:700,margin:"0 auto",padding:"40px 24px 100px"}}>
          <button onClick={()=>setView("landing")} style={B({background:"#F3F4F6",color:"#374151",padding:"8px 16px",fontSize:13,marginBottom:24,display:"flex",alignItems:"center",gap:6})}>Back</button>
          <div style={{background:"#FFF",borderRadius:20,padding:"32px 28px",border:"1px solid #F3F4F6"}}>
            <div style={{textAlign:"center",marginBottom:28}}>
              ...
              <h1 style={{fontSize:26,fontWeight:800,marginBottom:4}}>Terms and Conditions</h1>
            ...
            {[
              ...
              {icon:Calendar,     title:"3. Loan Wapsi",            points:["3, 6, ya 12 months mein wapsi.","Monthly installment waqt pe.","Bar waqt ada karne wale dubara eligible."]},
              {icon:AlertTriangle,title:"4. Default",               points:["Reminder phir formal notice phir qanooni karwai.","CNIC record mein default mark.","Future loans ke liye INELIGIBLE.","Recovery agency se wasool."]},
              {icon:RefreshCw,    title:"5. Dubara Loan",           points:["Pehla waqt pe ada karo phir dubara apply karo.","Achi history se zyada raqam.","Default se dobara eligible NAHI."]},
            ...
            <div style={{background:"#FEF2F2",border:"1px solid #FCA5A5",borderRadius:14,padding:"18px 20px",marginTop:20}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                ...
                <div style={{fontWeight:700,fontSize:15,color:"#DC2626"}}>Important Notice</div>
              </div>
              <div style={{fontSize:13,color:"#991B1B",lineHeight:1.7}}>
                Loan wapas na karna ek <strong>qanooni jurm</strong> hai. Aapka <strong>CNIC, phone, address</strong> record mein hai. Default pe <strong>qanooni karwai</strong> aur <strong>recovery</strong> hogi.
              </div>
            </div>
            <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:14,padding:"18px 20px",marginTop:14}}>
              ...
              <div style={{fontSize:13,color:"#166534",lineHeight:1.8}}>Pehla loan waqt pe ada karein. Installments time pe bhejein. Dubara apply karein!</div>
            </div>
              ...
              <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:"14px 16px",marginBottom:16,marginTop:8}}>
                <label style={{display:"flex",alignItems:"start",gap:10,cursor:"pointer"}}>
                  ...
                  <span style={{fontSize:13,color:"#92400E",lineHeight:1.6}}>
                    Main ne <strong style={{color:"#0D9488",cursor:"pointer",textDecoration:"underline"}} onClick={(e)=>{e.preventDefault();setView("terms");}}>Terms and Conditions</strong> padh li hain. Loan wapas na karne ki soorat mein <strong>qanooni karwai</strong> ho sakti hai.
                  </span>
                </label>
              </div>
              <button type="submit" disabled={submitting||!agreed} style={B({width:"100%",background:agreed?"linear-gradient(135deg,#0D9488,#0F766E)":"#D1D5DB",color:"#FFF",padding:"15px",fontSize:16,marginTop:4,opacity:submitting?0.7:1})}>
                {submitting?"Submit ho raha hai...":"Submit and Pay Rs. 350"}
              </button>
        ...
        <div style={{maxWidth:480,margin:"0 auto",padding:"50px 24px 100px",textAlign:"center"}}>
          ...
          <div style={{background:"#FFF",border:"2px solid #0D9488",borderRadius:20,padding:28,textAlign:"left"}}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={{fontSize:11,color:"#9CA3AF",marginTop:4}}>JazzCash app mein Transaction Details mein milega</div>
            </div>
      ...
      {view==="done"&&(
        <div style={{maxWidth:480,margin:"0 auto",padding:"80px 24px",textAlign:"center"}}>
          ...
          <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:14,marginBottom:20,fontSize:13,color:"#92400E"}}>
            Loan bar waqt ada karein — dubara eligible rahein!
          </div>
      ...
      {view==="admin-login"&&(
        <div style={{maxWidth:380,margin:"0 auto",padding:"100px 24px",textAlign:"center"}}>
          ...
          <p style={{color:"#9CA3AF",fontSize:13,marginBottom:20}}>Firebase Authentication</p>
          <div style={{textAlign:"left",marginBottom:12}}>
            <span style={{display:"flex",alignItems:"center",gap:6,fontSize:13,color:"#6B7280",marginBottom:6,fontWeight:500}}>
              <Mail size={14} color="#0D9488"/> Email
            </span>
            <input type="email" style={inp} placeholder="admin@aasanpay.com" value={adminEmail}
              onChange={e=>setAdminEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&checkAdmin()}/>
          </div>
          <div style={{textAlign:"left",marginBottom:16}}>
            <span style={{display:"flex",alignItems:"center",gap:6,fontSize:13,color:"#6B7280",marginBottom:6,fontWeight:500}}>
              <Lock size={14} color="#0D9488"/> Password
            </span>
            <input type="password" style={inp} placeholder="Password" value={adminPass}
              onChange={e=>setAdminPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&checkAdmin()}/>
          </div>
          {passErr&&<div style={{color:"#DC2626",fontSize:13,marginBottom:12}}>{passErr}</div>}
          <button onClick={checkAdmin} disabled={loginLoading} style={B({width:"100%",background:"linear-gradient(135deg,#0D9488,#0F766E)",color:"#FFF",padding:"13px",fontSize:15,opacity:loginLoading?0.7:1})}>
      ...
      {view==="admin"&&(
        authChecked && !authUser ? (
          <div style={{textAlign:"center",padding:100,color:"#9CA3AF"}}>Login karein...</div>
        ):(
        <div style={{maxWidth:1100,margin:"0 auto",padding:"40px 24px 100px"}}>
          <div style={{background:"#F0FDFA",border:"1px solid #99F6E4",borderRadius:12,padding:"12px 18px",marginBottom:20,fontSize:13,color:"#0F766E",display:"flex",alignItems:"center",gap:8}}>
            <strong>Firebase Connected</strong> — {authUser?.email}
          </div>
              ...
              {apps.map((a,i)=>{
                ...
                return(
                  <div key={a.id} onClick={()=>setSel(a)}
                    ...
                    <div style={{color:a.tid?"#16A34A":"#DC2626",fontSize:11,fontWeight:600}}>{a.tid?"TID OK":"No TID"}</div>
