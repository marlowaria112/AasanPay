...
import {
  ...
  Shield, Heart, Sparkles, Gift, HeartHandshake, FileText,
...
export default function AasanPay(){
  ...
  // ── Firebase Auth: track login state ──────────────────────────────
  useEffect(()=>{
    const unsub = onAuthStateChanged(auth, (user)=>{
      setAuthUser(user);
      setAuthChecked(true);
      if(user && view==="admin-login"){ setView("admin"); }
      if(!user && view==="admin"){ setView("admin-login"); }
    });
    return ()=>unsub();
  ...
  async function checkAdmin(){
    if(!adminEmail.trim()||!adminPass.trim()){ setPassErr("Email aur password dono likhein."); return; }
    ...
    <div style={{ minHeight:"100vh",background:"#FAFDFB",color:"#111827",fontFamily:"'Inter',sans-serif" }}>
      ...
      {view==="landing"&&<>
        ...
        <div style={{ maxWidth:900,margin:"32px auto 0",padding:"0 24px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:14 }}>
          {[
            ...
            { icon:HeartHandshake, value:"Rs. 0",                            label:"Interest — ZERO",    color:"#16A34A" },
      ...
      {view==="admin-login"&&(
        <div style={{ maxWidth:380,margin:"0 auto",padding:"100px 24px",textAlign:"center" }}>
          ...
          <p style={{ color:"#9CA3AF",fontSize:13,marginBottom:20 }}>Firebase Authentication se protected</p>
          ...
          <button onClick={checkAdmin} disabled={loginLoading} style={B({ width:"100%",background:"linear-gradient(135deg,#0D9488,#0F766E)",color:"#FFF",padding:"13px",fontSize:15,opacity:loginLoading?.7:1 })}>
      ...
      {view==="admin"&&(
        authChecked && !authUser ? (
          <div style={{ textAlign:"center",padding:100,color:"#9CA3AF" }}>Redirecting to login...</div>
        ) : (
        <div style={{ maxWidth:1100,margin:"0 auto",padding:"40px 24px 100px" }}>
          ...
          <div style={{ background:"#F0FDFA",border:"1px solid #99F6E4",borderRadius:12,padding:"12px 18px",marginBottom:20,fontSize:13,color:"#0F766E",display:"flex",alignItems:"center",gap:8 }}>
            🔥 <strong>Firebase Connected</strong> — Logged in as <strong>{authUser?.email}</strong> ✅
          </div>
