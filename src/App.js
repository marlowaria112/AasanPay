...
import {
  ...
  Shield, Heart, Sparkles, Gift, FileText, Search,
    ...
    <div style={{ background:"#FFF",borderRadius:16,padding:24,border:"1px solid #E2E8F0",boxShadow:"0 2px 8px rgba(0,0,0,.04)" }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"start",marginBottom:12 }}>
        <div style={{ display:"flex",gap:2 }}>
          {Array.from({length:r.stars}).map((_,j)=><Star key={j} size={14} color="#F59E0B" fill="#F59E0B"/>)}
        </div>
...
const B=(x={})=>({ border:"none",borderRadius:10,fontWeight:600,cursor:"pointer",fontFamily:"'Inter',sans-serif",...x });
...
export default function AasanPay(){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#F8FAFC",color:"#111827",fontFamily:"'Inter',sans-serif"}}>
      <style>{FONT+"*{box-sizing:border-box;margin:0}input:focus,select:focus{border-color:#0D9488!important;outline:none}"}</style>

      {/* NAV */}
      <nav style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 32px",background:"#FFF",borderBottom:"1px solid #E2E8F0",position:"sticky",top:0,zIndex:40,boxShadow:"0 1px 3px rgba(0,0,0,.06)"}}>
        <div onClick={()=>setView("landing")} style={{cursor:"pointer",display:"flex",alignItems:"center",gap:10,fontWeight:800,fontSize:22}}>
          <div style={{background:"linear-gradient(135deg,#0D9488,#0F766E)",borderRadius:10,width:38,height:38,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 8px rgba(13,148,136,.3)"}}>
            <Wallet size={20} color="#FFF"/>
          </div>
          <span>Aasan<span style={{color:"#0D9488"}}>Pay</span></span>
          <span style={{background:"#ECFDF5",color:"#059669",fontSize:10,padding:"3px 8px",borderRadius:6,fontWeight:700,border:"1px solid #A7F3D0",letterSpacing:.3}}>HALAAL</span>
        </div>
        <div style={{display:"flex",gap:16,alignItems:"center"}}>
          <button onClick={()=>setView("status")} style={B({background:"#F0FDF4",color:"#0D9488",padding:"8px 16px",fontSize:13,border:"1px solid #A7F3D0",display:"flex",alignItems:"center",gap:6})}>
            <Search size={13}/> Track Application
          </button>
          <button onClick={()=>setView("terms")} style={{background:"none",border:"none",color:"#9CA3AF",fontSize:12,cursor:"pointer"}}>Terms</button>
      ...
      {view==="landing"&&<>
        ...
        {/* HERO */}
        <section style={{background:"linear-gradient(135deg,#F0FDFA 0%,#ECFDF5 50%,#F8FAFC 100%)",padding:"56px 24px 52px",textAlign:"center",borderBottom:"1px solid #E2E8F0"}}>
          <div style={{marginBottom:20}}>
            <Ticker/>
          </div>

          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"#FFF",border:"1px solid #A7F3D0",borderRadius:24,padding:"8px 20px",fontSize:13,color:"#065F46",fontWeight:600,marginBottom:24,boxShadow:"0 1px 4px rgba(0,0,0,.06)"}}>
            <BadgeCheck size={15} color="#059669"/> Pakistan ka #1 Halaal Micro-Finance Platform
          </div>

          <h1 style={{fontSize:"clamp(30px,5.5vw,52px)",fontWeight:900,lineHeight:1.1,marginBottom:16,letterSpacing:-1.5,color:"#0F172A"}}>
          ...
          <p style={{fontSize:16,color:"#475569",maxWidth:520,margin:"0 auto 28px",lineHeight:1.75}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:10,marginBottom:32,flexWrap:"wrap"}}>
            {[
              {icon:"✓", text:"Zero Interest"},
              {icon:"✓", text:"No Hidden Fees"},
              {icon:"✓", text:"Shariah Compliant"},
              {icon:"✓", text:"4-6 Hour Approval"},
            ].map((b,i)=>(
              <span key={i} style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:20,padding:"7px 16px",fontSize:12.5,color:"#374151",fontWeight:500,display:"flex",alignItems:"center",gap:5}}>
          ...
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            <button onClick={()=>setView("apply")} style={B({background:"linear-gradient(135deg,#0D9488,#0F766E)",color:"#FFF",padding:"16px 36px",fontSize:16,display:"inline-flex",alignItems:"center",gap:8,boxShadow:"0 4px 20px rgba(13,148,136,.35)",borderRadius:12})}>
              Apply Now — Free <ArrowRight size={18}/>
            </button>
            <button onClick={()=>setView("status")} style={B({background:"#FFF",color:"#0D9488",padding:"16px 28px",fontSize:15,border:"2px solid #0D9488",display:"inline-flex",alignItems:"center",gap:8,borderRadius:12})}>
        ...
        <div style={{background:"#FFF",borderBottom:"1px solid #E2E8F0"}}>
          <div style={{maxWidth:900,margin:"0 auto",padding:"28px 24px",display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:0}}>
            {[
              {icon:Users,   value:<CountUp target={1500} suffix="+"/>, label:"Verified Borrowers",  sub:"Since 2023",       color:"#0D9488"},
              ...
              <div key={i} style={{textAlign:"center",padding:"16px 8px",borderRight:i<4?"1px solid #F1F5F9":"none"}}>
        ...
        {/* LOAN CALCULATOR */}
        <div style={{maxWidth:640,margin:"44px auto 0",padding:"0 24px"}}>
          <div style={{background:"#FFF",borderRadius:20,padding:32,border:"1px solid #E2E8F0",boxShadow:"0 4px 20px rgba(0,0,0,.06)"}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
              <div style={{width:36,height:36,background:"linear-gradient(135deg,#0D9488,#14B8A6)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center"}}>
                <TrendingUp size={18} color="#FFF"/>
              </div>
              <div>
                <div style={{fontWeight:700,fontSize:17}}>Loan Calculator</div>
                <div style={{fontSize:12,color:"#9CA3AF"}}>See exactly what you'll pay back</div>
              </div>
            </div>
            ...
            <div style={{marginTop:24}}>
              ...
              <input type="range" min={5000} max={35000} step={1000} value={slider} onChange={e=>setSlider(+e.target.value)} style={{width:"100%",accentColor:"#0D9488",marginBottom:6,height:6}}/>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:24}}>
                {[3,6,12].map(m=>(
                  <button key={m} onClick={()=>setTenure(m)} style={B({padding:"10px",fontSize:14,fontWeight:tenure===m?700:500,background:tenure===m?"linear-gradient(135deg,#0D9488,#0F766E)":"#F8FAFC",color:tenure===m?"#FFF":"#374151",border:tenure===m?"none":"1px solid #E2E8F0",borderRadius:10})}>
                    {m} Months
                  </button>
                ))}
              </div>
              ...
              <div style={{background:"linear-gradient(135deg,#F0FDFA,#ECFDF5)",border:"1px solid #A7F3D0",borderRadius:14,padding:20}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,textAlign:"center"}}>
                  <div>
                    <div style={{fontSize:11,color:"#6B7280",marginBottom:4,fontWeight:500}}>MONTHLY</div>
                    <div style={{fontSize:20,fontWeight:800,color:"#0D9488"}}>Rs. {monthly.toLocaleString()}</div>
                  </div>
                  <div style={{borderLeft:"1px solid #A7F3D0",borderRight:"1px solid #A7F3D0"}}>
                    <div style={{fontSize:11,color:"#6B7280",marginBottom:4,fontWeight:500}}>TOTAL PAYBACK</div>
                    <div style={{fontSize:20,fontWeight:800,color:"#0D9488"}}>Rs. {slider.toLocaleString()}</div>
                  </div>
                  <div>
                    <div style={{fontSize:11,color:"#6B7280",marginBottom:4,fontWeight:500}}>INTEREST</div>
                    <div style={{fontSize:20,fontWeight:800,color:"#059669"}}>Rs. 0</div>
                  </div>
                </div>
        ...
        <div style={{maxWidth:900,margin:"44px auto 0",padding:"0 24px"}}>
          <div style={{textAlign:"center",marginBottom:28}}>
            ...
            <h2 style={{fontSize:28,fontWeight:800,color:"#0F172A"}}>Built for those who need it most</h2>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:16}}>
            {[
              ...
              <div key={i} style={{background:c.bg,borderRadius:16,padding:22,border:`1px solid ${c.bd}`}}>
        ...
        <div style={{maxWidth:900,margin:"44px auto 0",padding:"0 24px"}}>
          <div style={{textAlign:"center",marginBottom:28}}>
            ...
            <h2 style={{fontSize:28,fontWeight:800,color:"#0F172A"}}>Simple. Fast. Transparent.</h2>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:16}}>
            {[
              ...
              <div key={i} style={{background:"#FFF",borderRadius:16,padding:22,border:"1px solid #E2E8F0",position:"relative",boxShadow:"0 2px 8px rgba(0,0,0,.03)"}}>
        ...
        {/* FEE SECTION */}
        <div style={{maxWidth:680,margin:"44px auto 0",padding:"0 24px"}}>
          <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:18,padding:28,boxShadow:"0 2px 8px rgba(0,0,0,.04)"}}>
        ...
        <div style={{maxWidth:1000,margin:"44px auto 0",padding:"0 24px"}}>
          <div style={{textAlign:"center",marginBottom:28}}>
            ...
            <h2 style={{fontSize:28,fontWeight:800,color:"#0F172A"}}>What our borrowers say</h2>
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:8}}>
              <div style={{display:"flex",gap:2}}>
                {Array.from({length:5}).map((_,j)=><Star key={j} size={16} color="#F59E0B" fill="#F59E0B"/>)}
              </div>
        ...
        <div style={{maxWidth:720,margin:"44px auto 0",padding:"0 24px"}}>
          <div style={{background:"linear-gradient(135deg,#0D9488,#065F46)",borderRadius:20,padding:"36px 32px",color:"#FFF"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:24,alignItems:"center"}}>
              <div>
                <div style={{fontSize:13,fontWeight:600,color:"#6EE7B7",marginBottom:6,letterSpacing:.5}}>HALAAL FINANCING</div>
                <h3 style={{fontSize:24,fontWeight:800,marginBottom:8,lineHeight:1.2}}>Ready to get started?</h3>
                ...
                <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
                  <button onClick={()=>setView("apply")} style={B({background:"#FFF",color:"#0D9488",padding:"13px 28px",fontSize:14,borderRadius:10,fontWeight:700})}>
                    Apply Now — Free
                  </button>
                  <button onClick={()=>setView("status")} style={B({background:"rgba(255,255,255,.15)",color:"#FFF",padding:"13px 24px",fontSize:14,border:"1px solid rgba(255,255,255,.3)",borderRadius:10})}>
                    Track Application
                  </button>
                </div>
              </div>
              <div style={{textAlign:"center",display:"grid",gap:12}}>
                {[
                  ...
                  <div key={i} style={{background:"rgba(255,255,255,.1)",borderRadius:12,padding:"12px 20px",textAlign:"center"}}>
        ...
        <div style={{maxWidth:640,margin:"44px auto 0",padding:"0 24px"}}>
          <div style={{textAlign:"center",marginBottom:24}}>
            <h2 style={{fontSize:24,fontWeight:800,color:"#0F172A"}}>Frequently Asked Questions</h2>
          </div>
          {[
            ...
            {q:"What happens if I can't repay on time?",           a:"Contact us immediately. Deliberate non-payment may result in legal proceedings under Pakistani law."},
          ].map((f,i)=>(
      ...
      {view==="status"&&(
        <div style={{maxWidth:560,margin:"0 auto",padding:"50px 24px 100px"}}>
          ...
          <div style={{textAlign:"center",marginBottom:32}}>
            ...
            <h2 style={{fontSize:26,fontWeight:800,marginBottom:6,color:"#0F172A"}}>Track Your Application</h2>
            <p style={{color:"#64748B",fontSize:14}}>Enter your Application ID to check real-time status</p>
          </div>

          <div style={{background:"#FFF",borderRadius:18,padding:28,border:"1px solid #E2E8F0",boxShadow:"0 4px 16px rgba(0,0,0,.05)"}}>
            <div style={{marginBottom:16}}>
              ...
              <div style={{display:"flex",gap:10}}>
                <input
                  ...
                  style={{...inp,flex:1,textTransform:"uppercase",letterSpacing:1,fontWeight:600}}
                  placeholder="AP-2025-XXXX"
                />
                <button onClick={checkStatus} disabled={statusLoading} style={B({background:"linear-gradient(135deg,#0D9488,#0F766E)",color:"#FFF",padding:"13px 20px",fontSize:14,borderRadius:10,opacity:statusLoading?.7:1})}>
                  {statusLoading?"...":"Check"}
                </button>
              </div>
            </div>
              ...
              return(
                <div style={{marginTop:8}}>
                  <div style={{background:st.bg,border:`1px solid ${st.bd}`,borderRadius:14,padding:20,marginBottom:16,textAlign:"center"}}>
                    ...
                    <div style={{fontSize:26,fontWeight:800,color:st.fg,marginBottom:4}}>{st.label}</div>
                    <div style={{fontSize:12,color:"#6B7280"}}>ID: {statusResult.applicationId}</div>
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                    {[
                      ...
                      <div key={k} style={{background:"#F8FAFC",borderRadius:10,padding:"12px 14px"}}>
                        <div style={{fontSize:10,color:"#94A3B8",fontWeight:500,marginBottom:3}}>{k.toUpperCase()}</div>
                  ...
                  {statusResult.status==="Pending"&&(
                    <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:"14px 16px",marginTop:16,fontSize:13,color:"#92400E"}}>
                      Your application is under review. Our team will contact you within 4-6 hours.
                    </div>
                  )}
                  {statusResult.status==="Approved"&&(
                    <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:12,padding:"14px 16px",marginTop:16,fontSize:13,color:"#166534"}}>
                      Congratulations! Your application is approved. Our team will contact you for disbursement.
                    </div>
                  )}
                  {statusResult.status==="Disbursed"&&(
                    <div style={{background:"#EFF6FF",border:"1px solid #BFDBFE",borderRadius:12,padding:"14px 16px",marginTop:16,fontSize:13,color:"#1E40AF"}}>
                      Funds have been transferred to your account. Please repay on time to stay eligible.
                    </div>
                  )}
                  {statusResult.status==="Rejected"&&(
                    <div style={{background:"#FEF2F2",border:"1px solid #FCA5A5",borderRadius:12,padding:"14px 16px",marginTop:16,fontSize:13,color:"#991B1B"}}>
                      Unfortunately your application was not approved this time. You may reapply after 30 days.
                    </div>
                  )}
                </div>
              );
      ...
      {view==="terms"&&(
        <div style={{maxWidth:700,margin:"0 auto",padding:"40px 24px 100px"}}>
          ...
          <div style={{background:"#FFF",borderRadius:20,padding:"32px 28px",border:"1px solid #E2E8F0",boxShadow:"0 4px 16px rgba(0,0,0,.05)"}}>
            <div style={{textAlign:"center",marginBottom:28}}>
              ...
              <h1 style={{fontSize:24,fontWeight:800,marginBottom:4,color:"#0F172A"}}>Terms and Conditions</h1>
            ...
            {[
              ...
              <div key={i} style={{marginBottom:24,paddingBottom:24,borderBottom:i<5?"1px solid #F1F5F9":"none"}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                  <div style={{width:32,height:32,borderRadius:8,background:"#F0FDFA",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid #A7F3D0"}}>
      ...
      {view==="apply"&&(
        <div style={{maxWidth:540,margin:"0 auto",padding:"44px 24px 100px"}}>
          <div style={{textAlign:"center",marginBottom:28}}>
            ...
            <h2 style={{fontSize:26,fontWeight:800,marginBottom:4,color:"#0F172A"}}>Apply for Halaal Financing</h2>
            <p style={{color:"#64748B",fontSize:14}}>Zero interest · Qarz-e-Hasna · Rs. 5,000 to Rs. 35,000</p>
          </div>

          <div style={{background:"#FFF",borderRadius:20,padding:28,border:"1px solid #E2E8F0",boxShadow:"0 4px 16px rgba(0,0,0,.05)"}}>
            <form onSubmit={submit}>
              <Field icon={User} label="Full Name (as per CNIC)"><input required style={inp} value={form.fullName} onChange={e=>setForm({...form,fullName:e.target.value})} placeholder="e.g. Muhammad Ali"/></Field>
              ...
              <Field icon={MapPin} label="City"><input required style={inp} value={form.city} onChange={e=>setForm({...form,city:e.target.value})} placeholder="e.g. Lahore"/></Field>
              ...
              <Field icon={Wallet} label="Loan Amount Required">
                <select required style={{...inp,cursor:"pointer"}} value={form.loanAmount} onChange={e=>setForm({...form,loanAmount:e.target.value})}>
                  ...
                  {[5000,7000,10000,12000,15000,18000,20000,25000,30000,35000].map(v=><option key={v} value={v}>Rs. {v.toLocaleString()}</option>)}
                </select>
              </Field>
              ...
              <Field icon={Landmark} label="JazzCash / Easypaisa Number (for disbursement)"><input required style={inp} value={form.easypaisa} onChange={e=>setForm({...form,easypaisa:e.target.value})} placeholder="03XX-XXXXXXX"/></Field>
              ...
              <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:"14px 16px",marginBottom:16,marginTop:8}}>
                <label style={{display:"flex",alignItems:"start",gap:10,cursor:"pointer"}}>
                  ...
                  <span style={{fontSize:13,color:"#78350F",lineHeight:1.6}}>
                    I have read and agree to the <strong style={{color:"#0D9488",cursor:"pointer",textDecoration:"underline"}} onClick={(e)=>{e.preventDefault();setView("terms");}}>Terms and Conditions</strong>. I understand that non-repayment may result in legal action.
                  </span>
                </label>
              </div>
      ...
      {view==="payment"&&(
        <div style={{maxWidth:520,margin:"0 auto",padding:"50px 24px 100px"}}>
          <div style={{textAlign:"center",marginBottom:28}}>
            ...
            <h2 style={{fontSize:24,fontWeight:800,marginBottom:6,color:"#0F172A"}}>Application Submitted!</h2>
            <p style={{color:"#64748B",fontSize:14}}>Save your Application ID — you will need it to track your status</p>
          </div>

          {/* APP ID BOX */}
          <div style={{background:"linear-gradient(135deg,#F0FDFA,#ECFDF5)",border:"2px solid #0D9488",borderRadius:16,padding:20,marginBottom:20,textAlign:"center"}}>
            <div style={{fontSize:11,fontWeight:600,color:"#6B7280",marginBottom:8,letterSpacing:1}}>YOUR APPLICATION ID</div>
            <div style={{fontSize:32,fontWeight:900,color:"#0D9488",letterSpacing:3,marginBottom:12,fontFamily:"monospace"}}>{appId}</div>
            <button onClick={copyAppId} style={B({background:idCopied?"#059669":"#0D9488",color:"#FFF",padding:"9px 22px",fontSize:13,display:"inline-flex",alignItems:"center",gap:6,borderRadius:8})}>
              {idCopied?<><Check size={14}/> Copied!</>:<><Copy size={14}/> Copy ID</>}
            </button>
            <div style={{fontSize:12,color:"#64748B",marginTop:10}}>Screenshot this or save it — use it to track your application</div>
          </div>

          {/* PAYMENT BOX */}
          <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:18,padding:28,boxShadow:"0 4px 16px rgba(0,0,0,.05)"}}>
            <div style={{fontWeight:700,fontSize:16,color:"#0F172A",marginBottom:4}}>Step 2 — Pay Verification Fee</div>
            <div style={{fontSize:13,color:"#64748B",marginBottom:20}}>Send Rs. 350 via JazzCash to complete your application</div>

            <div style={{background:"#F8FAFC",border:"1px solid #E2E8F0",borderRadius:14,padding:20,marginBottom:16}}>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
                <div>
                  <div style={{fontSize:10,color:"#94A3B8",fontWeight:600,marginBottom:4,letterSpacing:.5}}>JAZZCASH NUMBER</div>
                  <div style={{fontSize:18,fontWeight:800,color:"#0F172A",letterSpacing:1}}>{JAZZCASH}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div style={{fontSize:14,fontWeight:600,color:"#0F172A"}}>JazzCash Send Money</div>
                </div>
              </div>
            ...
            <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:12,padding:14,marginBottom:20,fontSize:13,color:"#166534"}}>
              <strong>How to pay:</strong> Open JazzCash app → Send Money → Enter number {JAZZCASH} → Amount Rs. 350 → Send → Copy Transaction ID below
            </div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <input
                ...
                style={{...inp,textAlign:"center",fontSize:17,fontWeight:700,letterSpacing:2}}
                placeholder="e.g. TXN1234567890"
              />
              <div style={{fontSize:11,color:"#9CA3AF",marginTop:4}}>Find in JazzCash app → Transactions → Latest</div>
            </div>
      ...
      {view==="done"&&(
        <div style={{maxWidth:520,margin:"0 auto",padding:"70px 24px 100px",textAlign:"center"}}>
          ...
          <h2 style={{fontSize:26,fontWeight:800,marginBottom:8,color:"#0F172A"}}>You're All Set!</h2>
          <p style={{color:"#64748B",fontSize:15,lineHeight:1.7,marginBottom:28}}>
            Your application and payment have been received. Our team will call you within 4-6 hours.
          </p>

          <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:16,padding:24,marginBottom:20,boxShadow:"0 2px 8px rgba(0,0,0,.04)"}}>
            <div style={{fontSize:11,fontWeight:600,color:"#6B7280",marginBottom:8,letterSpacing:1}}>YOUR APPLICATION ID</div>
            <div style={{fontSize:28,fontWeight:900,color:"#0D9488",letterSpacing:3,marginBottom:10,fontFamily:"monospace"}}>{appId}</div>
            ...
            {[
              ...
              <div key={i} style={{display:"flex",alignItems:"center",gap:10,marginBottom:i<4?10:0}}>
                <div style={{width:20,height:20,borderRadius:"50%",background:s.done?"#16A34A":"#E2E8F0",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                  {s.done?<Check size={11} color="#FFF"/>:<div style={{width:6,height:6,borderRadius:"50%",background:"#94A3B8"}}/>}
                </div>
          ...
          <div style={{display:"flex",gap:10,justifyContent:"center"}}>
            ...
            <button onClick={()=>{setForm({fullName:"",cnic:"",phone:"",city:"",category:"",purpose:"",loanAmount:"",tenure:"12",easypaisa:""});setAgreed(false);setAppId("");setView("landing");}}
              style={B({background:"#F1F5F9",color:"#374151",padding:"12px 20px",fontSize:14,borderRadius:10})}>
              Home
            </button>
          </div>
        </div>
      ...
      {view==="admin-login"&&(
        <div style={{maxWidth:380,margin:"0 auto",padding:"100px 24px",textAlign:"center"}}>
          ...
          <div style={{textAlign:"left",marginBottom:12}}>
            ...
            <input type="email" style={inp} placeholder="admin@aasanpay.com" value={adminEmail} onChange={e=>setAdminEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&checkAdmin()}/>
          </div>
          <div style={{textAlign:"left",marginBottom:20}}>
            ...
            <input type="password" style={inp} placeholder="Password" value={adminPass} onChange={e=>setAdminPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&checkAdmin()}/>
          </div>
          ...
          <div style={{maxWidth:1100,margin:"0 auto",padding:"40px 24px 100px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              <div>
                ...
                <div style={{fontSize:13,color:"#9CA3AF"}}>{authUser?.email}</div>
              </div>
              <div style={{background:"#F0FDFA",border:"1px solid #A7F3D0",borderRadius:10,padding:"8px 16px",fontSize:13,color:"#065F46",fontWeight:600}}>
                Firebase Connected
              </div>
            </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:14,marginBottom:24}}>
              {[
                ...
                {l:"Pending",  v:apps.filter(a=>a.status==="Pending").length,   c:"#EA580C",bg:"#FFF7ED",bd:"#FDBA74"},
                {l:"Approved", v:apps.filter(a=>a.status==="Approved").length,  c:"#16A34A",bg:"#F0FDF4",bd:"#86EFAC"},
                {l:"Disbursed",v:apps.filter(a=>a.status==="Disbursed").length, c:"#2563EB",bg:"#EFF6FF",bd:"#93C5FD"},
                {l:"Rejected", v:apps.filter(a=>a.status==="Rejected").length,  c:"#DC2626",bg:"#FEF2F2",bd:"#FCA5A5"},
              ].map((s,i)=>(
            ...
            {loading?(
              ...
              <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,.04)"}}>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 0.8fr 0.8fr 0.6fr 0.8fr 1fr auto",padding:"10px 18px",background:"#F8FAFC",borderBottom:"1px solid #E2E8F0",fontSize:11,fontWeight:600,color:"#94A3B8",letterSpacing:.5}}>
          ...
          <div onClick={e=>e.stopPropagation()} style={{background:"#FFF",borderRadius:20,padding:28,maxWidth:540,width:"100%",boxShadow:"0 25px 60px rgba(0,0,0,.2)",maxHeight:"90vh",overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:46,height:46,borderRadius:"50%",background:"linear-gradient(135deg,#0D9488,#14B8A6)",color:"#FFF",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:18}}>{sel.fullName?.[0]}</div>
                <div>
                  <div style={{fontSize:18,fontWeight:700,color:"#0F172A"}}>{sel.fullName}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,fontSize:14,marginBottom:16,background:"#F8FAFC",borderRadius:14,padding:20}}>
              {[
                ["Phone",       sel.phone],
                ["City",        sel.city],
                ["Amount",      "Rs. "+Number(sel.loanAmount).toLocaleString()],
                ["Tenure",      sel.tenure+" months"],
                ["JazzCash",    sel.easypaisa],
                ["Purpose",     sel.purpose],
                ...
                <div key={k}>
                  <div style={{color:"#94A3B8",fontSize:10,marginBottom:3,fontWeight:600,letterSpacing:.3}}>{k.toUpperCase()}</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
              ...
              <div style={{background:STATUS[sel.status]?.bg||"#F8FAFC",border:`1px solid ${STATUS[sel.status]?.bd||"#E2E8F0"}`,borderRadius:12,padding:14,textAlign:"center"}}>
                <div style={{fontSize:10,color:"#6B7280",marginBottom:4,fontWeight:600}}>CURRENT STATUS</div>
                <div style={{fontSize:14,fontWeight:800,color:STATUS[sel.status]?.fg||"#374151"}}>{STATUS[sel.status]?.label||sel.status}</div>
              </div>
            </div>

            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>updateStatus(sel.id,"Approved")} style={B({flex:1,background:"#16A34A",color:"#FFF",padding:"11px",display:"flex",alignItems:"center",justifyContent:"center",gap:6,borderRadius:10})}><CheckCircle2 size={15}/> Approve</button>
              <button onClick={()=>updateStatus(sel.id,"Disbursed")} style={B({flex:1,background:"#2563EB",color:"#FFF",padding:"11px",borderRadius:10})}>Disburse</button>
              <button onClick={()=>updateStatus(sel.id,"Rejected")} style={B({flex:1,background:"#DC2626",color:"#FFF",padding:"11px",display:"flex",alignItems:"center",justifyContent:"center",gap:6,borderRadius:10})}><XCircle size={15}/> Reject</button>
            </div>
          </div>
        </div>
      )}
