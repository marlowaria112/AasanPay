...
export default function AasanPay(){
  ...
  async function confirmPayment(){
    if(!tid.trim()){
      alert("Transaction ID likhein!");
      return;
    }
    try{
      if(lastDocId){
        await updateDoc(doc(db,"applications",lastDocId),{tid:tid});
      }
      setView("done");
      setTid("");
    }catch(err){
      alert("TID save error: "+err.message);
      console.error("TID error:",err);
    }
  }
        ...
        <div style={{maxWidth:700,margin:"0 auto",padding:"40px 24px 100px"}}>
          ...
          <div style={{background:"#FFF",borderRadius:20,padding:"32px 28px",border:"1px solid #F3F4F6"}}>
            ...
            <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:14,padding:"18px 20px",marginTop:14}}>
              ...
              <div style={{fontSize:13,color:"#166534",lineHeight:1.8}}>Pehla loan waqt pe ada karein. Installments time pe bhejein. Dubara apply karein — zyada raqam mil sakti hai!</div>
            </div>
