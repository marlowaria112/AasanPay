...
const PULSE_STYLE = "@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(.8)}}";
...
export default function AasanPay(){
  ...
  async function checkStatus(){
    ...
    try{
      ...
      else{ setStatusResult(Object.assign({id:snap.docs[0].id},snap.docs[0].data())); }
    ...
    <div style={{minHeight:"100vh",background:"#F8FAFC",color:"#111827",fontFamily:"'Inter',sans-serif"}}>
        ...
        <div style={{maxWidth:1000,margin:"44px auto 0",padding:"0 24px"}}>
          <div style={{textAlign:"center",marginBottom:28}}>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginTop:8}}>
              <div style={{display:"flex",gap:2}}>
                {Array.from({length:5}).map(function(_,j){ return <Star key={j} size={16} color="#F59E0B" fill="#F59E0B"/>; })}
              </div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:18}}>
            {visibleReviews.map(function(r,i){ return <ReviewCard key={i} r={r}/>; })}
          </div>
          {!showAllReviews&&(
          ...
          <div style={{background:"#FFF",borderRadius:18,padding:28,border:"1px solid #E2E8F0",boxShadow:"0 4px 16px rgba(0,0,0,.05)"}}>
            <div style={{marginBottom:16}}>
              ...
              <div style={{display:"flex",gap:10}}>
                <input value={statusId} onChange={function(e){setStatusId(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter")checkStatus();}} style={Object.assign({},inp,{flex:1,textTransform:"uppercase",letterSpacing:1,fontWeight:600})} placeholder="AP-2025-XXXX"/>
                <button onClick={checkStatus} disabled={statusLoading} style={B({background:"linear-gradient(135deg,#0D9488,#0F766E)",color:"#FFF",padding:"13px 20px",fontSize:14,borderRadius:10,opacity:statusLoading?0.7:1})}>
                  {statusLoading?"...":"Check"}
                </button>
              </div>
            </div>
                ...
                <div style={{marginTop:8}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                    {[
                      ...
                      return(
                        <div key={kv[0]} style={{background:"#F8FAFC",borderRadius:10,padding:"12px 14px"}}>
                          <div style={{fontSize:10,color:"#94A3B8",fontWeight:500,marginBottom:3}}>{kv[0].toUpperCase()}</div>
                          <div style={{fontSize:14,fontWeight:600,color:"#0F172A"}}>{kv[1]}</div>
                        </div>
                      );
                    })}
                  </div>
            ...
            <form onSubmit={submit}>
              ...
              <Field icon={Wallet} label="Loan Amount Required">
                <select required style={Object.assign({},inp,{cursor:"pointer"})} value={form.loanAmount} onChange={function(e){setForm(Object.assign({},form,{loanAmount:e.target.value}));}}>
                  ...
                  {[5000,7000,10000,12000,15000,18000,20000,25000,30000,35000].map(function(v){ return <option key={v} value={v}>Rs. {v.toLocaleString()}</option>; })}
                </select>
              </Field>
              ...
              <Field icon={Landmark} label="JazzCash / Easypaisa Number (for disbursement)"><input required style={inp} value={form.easypaisa} onChange={function(e){setForm(Object.assign({},form,{easypaisa:e.target.value}));}} placeholder="03XX-XXXXXXX"/></Field>
        ...
        <div style={{maxWidth:520,margin:"0 auto",padding:"50px 24px 100px"}}>
          ...
          <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:18,padding:28,boxShadow:"0 4px 16px rgba(0,0,0,.05)"}}>
            ...
            <div style={{background:"#F0FDF4",border:"1px solid #BBF7D0",borderRadius:12,padding:14,marginBottom:20,fontSize:13,color:"#166534"}}>
              <strong>How to pay:</strong> Open JazzCash app then Send Money then Enter number {JAZZCASH} then Amount Rs. 350 then Send then Copy Transaction ID below
            </div>
            <div style={{marginBottom:16}}>
              ...
              <div style={{fontSize:11,color:"#9CA3AF",marginTop:4}}>Find in JazzCash app then Transactions then Latest</div>
            </div>
      ...
      {view==="done"&&(
        <div style={{maxWidth:520,margin:"0 auto",padding:"70px 24px 100px",textAlign:"center"}}>
          ...
          <h2 style={{fontSize:26,fontWeight:800,marginBottom:8,color:"#0F172A"}}>You are All Set!</h2>
            ...
            {loading?(
              ...
              <div style={{background:"#FFF",border:"1px solid #E2E8F0",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 8px rgba(0,0,0,.04)"}}>
                ...
                {apps.map(function(a,i){
                  ...
                  return(
                    <div key={a.id} onClick={function(){setSel(a);}}
      ...
      {sel&&view==="admin"&&authUser&&(
        <div onClick={function(){setSel(null);}} style={{position:"fixed",inset:0,background:"rgba(15,23,42,.5)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,zIndex:50}}>
          <div onClick={function(e){e.stopPropagation();}} style={{background:"#FFF",borderRadius:20,padding:28,maxWidth:540,width:"100%",boxShadow:"0 25px 60px rgba(0,0,0,.2)",maxHeight:"90vh",overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",marginBottom:20}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:46,height:46,borderRadius:"50%",background:"linear-gradient(135deg,#0D9488,#14B8A6)",color:"#FFF",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:18}}>{sel.fullName&&sel.fullName[0]}</div>
                <div>
              ...
              <X size={20} color="#9CA3AF" style={{cursor:"pointer"}} onClick={function(){setSel(null);}}/>
            </div>
              ...
              {[
                ...
                return(
                  <div key={kv[0]}>
                    ...
                    <div style={{fontWeight:600,color:"#0F172A",fontSize:13}}>{kv[1]}</div>
                  </div>
                );
              })}
            </div>
